...
@Component({
    selector: 'jhi-contributeur',
    templateUrl: './contributeur.component.html'
})
export class ContributeurComponent implements OnInit, OnDestroy {
    ...
    trackId(index: number, item: Contributeur) {
        return item.id;
    }
    registerChangeInContributeurs() {
        this.eventSubscriber = this.eventManager.subscribe('contributeurListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.contributeurs.push(data[i]);
        }
    }
